...
pkill -n  -f "/usr/lib/libreoffice" > /dev/null 2>&1

SAVE_DIR="${ARCHIVE}/QRV/${MYCALL}/SAVED/${MODULE_NAME}"
mkdir -p "${SAVE_DIR}"

mkdir -p "${SAVE_DIR}/config/libreoffice"
unlink "$HOME/.config/libreoffice"
rm -rf "$HOME/.config/libreoffice"
ln -sTf "${SAVE_DIR}/config/libreoffice" "$HOME/.config/libreoffice"
